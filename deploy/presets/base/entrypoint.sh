...
echo "[hada-preset] Starting preset container..."
echo "[hada-preset] Frontend port: 8080"
echo "[hada-preset] Workspace file server port: 9091"
...
if [ -d "/frontend/dist" ] && [ "$(ls -A /frontend/dist 2>/dev/null)" ]; then
    ...
    echo "[hada-preset] Frontend serving from /frontend/dist"
else
